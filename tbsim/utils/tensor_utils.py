...
def clone(x, ignore_if_unspecified= False):
    """
...
def gather_from_start_single(tensor,indices):
    if tensor.ndim<indices.ndim:
        return tensor
    gather_dim = indices.ndim-1
    
    desired_shape = list(indices.shape)+([1]*(tensor.ndim-gather_dim-1))
    repeats = [1]*indices.ndim+list(tensor.shape[indices.ndim:])
    indices = indices.reshape(desired_shape).repeat(repeats)
    if isinstance(tensor,torch.Tensor):
        return torch.gather(tensor,gather_dim,indices)
    elif isinstance(tensor,np.ndarray):
        return np.take(tensor,indices,axis=gather_dim)

def gather_from_start(tensor,indices):
    return recursive_dict_list_tuple_apply(
        tensor,
        {
            torch.Tensor: lambda x: gather_from_start_single(x,indices),
            np.ndarray: lambda x: gather_from_start_single(x,indices),
            type(None): lambda x: x,
        }
    )
...
def cat_list_of_dict(x,dim):
    """combining a list of dictionaries to a single dictionary by concatenating the values

    Args:
        x (List[Dict[tensor.Torch]]): _description_
    """
    combined_dict = dict()
    for k,v in x[0].items():
        if isinstance(v,torch.Tensor):
            if v.ndim>=dim:
                combined_dict[k] = torch.cat([xi[k] for xi in x],dim=dim)

        elif isinstance(v,dict):
            combined_dict[k] = cat_list_of_dict([xi[k] for xi in x],dim)
    return combined_dict

def block_diag_from_cat(x):
    """ convert a concatenated array to block diagonal

    Args:
        x (Union[torch.Tensor,np.ndarray]): [B,M,n,n]
    Returns:
        mat: [B,Mxn,Mxn]
    """
    M = x.shape[1]
    Id = torch.eye(M,device=x.device)
    slices = [torch.kron(Id[i], x[:, i]) for i in range(M)]
    mat = torch.cat(slices, 1)
    return mat

def compare_tensors_ignore_nan(tensor1, tensor2):
    # Create boolean masks for NaN values in both tensors
    mask1 = torch.isnan(tensor1)
    mask2 = torch.isnan(tensor2)

    # Apply the masks to both tensors to remove NaN values
    tensor1_no_nan = tensor1.clone().masked_fill_(mask1, 0)
    tensor2_no_nan = tensor2.clone().masked_fill_(mask2, 0)

    # Compare the non-NaN elements in both tensors using torch.equal()
    return torch.equal(tensor1_no_nan, tensor2_no_nan)