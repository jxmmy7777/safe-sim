...
def closest_points_on_centerline(agent_traj, centerline_traj, window_size=10):
    _, norm_dist, _ = GeoUtils.batch_proj(agent_traj[None, :], centerline_traj[...,[0,1,3]])
    norm_dist_indices = np.argmin(norm_dist, axis=-1).item()
    
    NORM_DIST_THRESHOLD = 5.0
    if norm_dist[0, norm_dist_indices] > NORM_DIST_THRESHOLD:
        return None

    start_idx = max(0, norm_dist_indices - window_size // 2)
    end_idx = min(centerline_traj.shape[0], norm_dist_indices + window_size // 2 + 1)

    windowed_centerline = centerline_traj[start_idx:end_idx, :]

    return windowed_centerline