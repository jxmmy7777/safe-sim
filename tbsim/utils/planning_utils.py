...
def get_collision_dis(
    ...
    col_loss = torch.zeros([B, N,T]).to(ego_trajectories.device)
...
def get_collision_loss(
    ...
    for et, func in col_funcs.items():
        ...
        if dis.nelement() > 0:
            if no_mask:
                col_loss += torch.sum(torch.sigmoid(-dis*4), dim=[2,3])/T
            else:
                col_loss += torch.sum(torch.sigmoid(-dis*4) * type_mask[et][:,None,:,None], dim=[2,3])/T
    
    return col_loss#.reshape(B*N,T)


def get_drivable_area_loss(
    ego_trajectories, raster_from_agent, dis_map, ego_extents, enable_grad=False
):
    """Cost for road departure."""
    with torch.set_grad_enabled(enable_grad):
        # print("dis_map dtype:", dis_map.dtype)
        # print("ego_trajectories dtype:", ego_trajectories.dtype)
        # print("raster_from_agent dtype:", raster_from_agent.dtype)
        # print("ego_extents dtype:", ego_extents.dtype)
        ego_trajectories = ego_trajectories.to(dis_map.dtype)
...
def get_lane_loss_simple(ego_trajectories, raster_from_agent, dis_map: torch.Tensor):
    from tbsim.models.cnn_roi_encoder import bilinear_interpolate,bilinear_interpolate_batch
    with torch.set_grad_enabled(True):
        dis_map.requires_grad_()
        h,w = dis_map.shape[-2:]
        
        raster_xy = GeoUtils.transform_points_tensor(ego_trajectories[...,:2],raster_from_agent)
        # raster_xy[...,0] = raster_xy[...,0].clip(0,w-1e-5)
        # raster_xy[...,1] = raster_xy[...,1].clip(0,h-1e-5)
        res = []
        # for i in range(raster_xy.shape[0]):
        #     x_sample = raster_xy[i]
        #     y_sample = raster_xy[i]
        #     res.append(
        #         bilinear_interpolate(dis_map[i], x_sample, y_sample)
        #     )
        res_batch =  bilinear_interpolate_batch(dis_map, raster_xy[...,0], raster_xy[...,1])
        # lane_loss = res_batch.sum(dim=-1)
...
def ego_sample_planning(
    ...
    if agent_trajectories.shape[1] ==0:
        col_loss=0
    else:
    ...
    if hasattr(log_likelihood,"ndim"):
