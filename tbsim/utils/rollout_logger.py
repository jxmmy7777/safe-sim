...
class RolloutLogger(object):
    ...
    def _combine_obs(self, obs):
        ...
        if "ego" in obs and obs["ego"] is not None:
            # Update combined with all keys from ego except 'extras'
            combined.update({k: v for k, v in obs["ego"].items() if k != "extras"})
            combined["exceed_lane"] = np.concatenate((obs["ego"]["extras"]["exceed_lane"],obs["agents"]["extras"]["exceed_lane"]),axis=0)
        if "agents" in obs and obs["agents"] is not None:
            for k in obs["agents"].keys():
                ...
                if k in combined:
                    ...
                    combined[k] = obs["agents"][k]
        #additionalyy store exceede lane
    ...
    def _combine_action(self, action: RolloutAction):
        ...
        if action.has_ego and not action.has_agents:
            ...
            for k in agents_action:
                if k in ego_action:
                    combined["action"][k] = np.concatenate((ego_action[k], agents_action[k]), axis=0) #TODO is this the correct way to save ego??
            if action.agents_info is not None and action.ego_info is not None:
                ...
                if ego_samples is not None and agents_samples is None:
                    ...
                    combined["action_samples"] = dict()
                    combined["action_denoising_samples"] = dict()
                    for k in agents_samples:
                        ...
                        #Add denoising information if needed
                    if action.agents_info.get("denoising_predictions") is not None:
                        for k in action.agents_info["denoising_predictions"]:
                            pad_k = np.zeros([Nego,*action.agents_info["denoising_predictions"][k].shape[1:]])
                            combined["action_denoising_samples"][k]=np.concatenate((pad_k,action.agents_info["denoising_predictions"][k]),0)
                    if action.agents_info.get("adv_proposals") is not None:
                        combined["adv_proposals"] = action.agents_info["adv_proposals"]
    ...
    def _append_buffer(self, obs, action):
        ...
        if "exceed_lane" in obs:
            state["exceed_lane"] = obs["exceed_lane"]
        ...
        if "action_denoising_samples" in action:
            if "positions" in action["action_denoising_samples"]:
                state["action_denoising_sample_positions"] = action["action_denoising_samples"]["positions"]
                state["action_denoising_sample_yaws"] = action["action_denoising_samples"]["yaws"]
            if "adv_proposals" in action["action_denoising_samples"]:
                state["adv_proposals"] =  action["action_denoising_samples"]["adv_proposals"]
        for i,si in enumerate(self._scene_indices):
            self._agent_id_per_scene[si] = obs["track_id"][obs["scene_index"] == si]
    ...
    def log_step(self, obs, action: RolloutAction):
        ...
        combined_action = self._combine_action(action) #This put all ego index to first few and then agents
        # TODO important! if batch >1 and ego is not zero, the combined
