...
class BatchUtils(object):
    ...
    def get_current_states(batch: dict, dyn_type: dynamics.DynType) -> torch.Tensor:
        ...
        # elif dyn_type == dynamics.DynType.FRENET:
        #     current_states = torch.zeros(bs, 6).to(batch["curr_speed"].device)  # [x, y, vel, yaw, n0, t0]
        #     current_states[:, 2] = batch["curr_speed"]
        #     current_states[:, 3] = batch["curr_yaw"]
        #     # Assuming batch contains 'n0' and 't0' for Frenet coordinates
        #     current_states[:, 4] = batch["n0"]
        #     current_states[:, 5] = batch["t0"]
        else:
