...
class HardcodeNuscPlanner():
    def __init__(self, lane_graphs, cfg):
        # super(HardcodeNuscPlanner, self).__init__(map_env, cfg)
        self.cfg = cfg
        self.lane_graphs = lane_graphs
    ...
    def reset(self, init_state, vehicle_atts, batch_mask, batch_size, batch_map_names, ego_idx=0):
        '''
        ...
        self.batch_maps = batch_map_names
...
def polyline_length(pts):
    return np.sum(np.sqrt(np.sum((pts[1:]-pts[:-1])**2,axis=1)))
...
def process_lanegraph(nmap, res_meters, eps):
    ...
    lane_dict = list(nmap.elements.values())[0]
    lane_graph = {}


    # discretize the lanes (each lane has at least 2 points)
    for lane in nmap.lanes:
        lane_len = int(np.rint(polyline_length(lane.center.points)))
        discretized = lane.center.interpolate(num_pts=max(lane_len,2)).points[:, :2]
        ...
        lane_graph[lane.id] = discretized


    # make sure connections don't have duplicates (now each lane has at least 1 point)
    for intok,conn in lane_dict.items():
        for outtok in conn.next_lanes:
    ...
    for lid,lane in lane_graph.items():
        ...
        for outtok in lane_dict[lid].next_lanes:
            if outtok in lane_graph:
                out_edges[laneid2start[lid]+len(lane)-1].append(laneid2start[outtok])
        for intok in lane_dict[lid].prev_lanes:
    ...
    return {'xy': np.array(xys), 'in_edges': in_edges, 'out_edges': out_edges, 'edges': edges, 'edgeixes': edgeixes, 'ee2ix': ee2ix}



def load_lane_graphs_trajdata(dataset, cache_path="~/.unified_data_cache", flip_singapore=False):    

    cache_path = Path(cache_path).expanduser()

    map_api = MapAPI(cache_path)
    if dataset=='nuplan':
        maps = {map_name: map_api.get_map('nuplan_mini:{}'.format(map_name)) for map_name in ["boston", "singapore", "pittsburgh", "las_vegas"]}
    elif dataset=='nuscenes':
        maps = {map_name: map_api.get_map('nusc_trainval:{}'.format(map_name)) for map_name in ["boston-seaport", "singapore-onenorth", "singapore-hollandvillage", "singapore-queenstown"]}
    
    print('Loading lane graphs...')
    lane_graphs = {map_name: process_lanegraph(nmap, 1.0, 1e-6) for map_name,nmap in maps.items()}
    
    # flip about x axis if desired (i.e. switch driving direction)
    map_list = list(maps.keys())
    for midx, mname in enumerate(map_list):   
        if flip_singapore and mname.split('-')[0] == 'singapore':
            print('Flipping %s about x axis...' % (mname))
            # also need to flip lane graph
            cur_lg = lane_graphs[mname]
            # xys is (L, 2), reflect y
            xys = cur_lg['xy']
            mheight = nmap.extent[3]-nmap.extent[0]
            xys[:,1] = mheight - xys[:,1]
            lane_graphs[mname]['xy'] = xys
            # negate diffy in edges [x0, y0, diff[0], diff[1], dist]
            edges = cur_lg['edges']
            edges[:,1] = mheight - edges[:,1]
            edges[:,3] *= -1
            lane_graphs[mname]['edges'] = edges
