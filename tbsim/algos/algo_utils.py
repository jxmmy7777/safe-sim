...
def yaw_from_pos(pos: torch.Tensor, dt, yaw_correction_speed=0., max_yaw_one_step=90, mode = "append_front"):
    """
    ...
    if mode =="append_front":
        delta_yaw = torch.cat((yaw[..., [0]], yaw[..., 1:] - yaw[..., :-1]), dim=-1)
    else:
        delta_yaw = torch.cat((yaw[..., 1:] - yaw[..., :-1], torch.zeros_like(yaw[..., [-1]])), dim=-1)
    ...
    delta_yaw[speed < yaw_correction_speed] = 0.
     ...
     ## If yaw is greater than 30 between two time steps degree turn to 0
    is_yaw_greater_than_90 = torch.abs(delta_yaw) > (torch.pi * max_yaw_one_step/ 180)
    delta_yaw[is_yaw_greater_than_90] = 0.
