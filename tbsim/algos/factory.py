...
def algo_factory(config: ExperimentConfig, modality_shapes: dict):
    ...
    if algo_name == "bc":
        ...
        algo = DiffusionTrafficModel(algo_config=algo_config, modality_shapes=modality_shapes)
    elif algo_name == "vae":
    ...
    elif algo_name == "diffscene": # diffscene is essentially 1)no dynamics model 2) different guidances
        algo = DiffusionTrafficModel(algo_config=algo_config, modality_shapes=modality_shapes)
    else:
