...
def bilinear_interpolate_batch(img, x, y, floattype=torch.float, flip_y=False):
    """Return bilinear interpolation of 4 nearest pts w.r.t to x,y from img
    Args:
        img (torch.Tensor): Tensor of size cxwxh. Usually one channel of feature layer
        x (torch.Tensor): Float dtype, x axis location for sampling
    ...
    Ia = img[torch.arange(bs).unsqueeze(-1), y0, x0]
    Ib = img[torch.arange(bs).unsqueeze(-1), y1, x0]
    Ic = img[torch.arange(bs).unsqueeze(-1), y0, x1]
    Id = img[torch.arange(bs).unsqueeze(-1), y1, x1]
...
def bilinear_interpolate_batch(img, x, y, floattype=torch.float, flip_y=False):
    """Return bilinear interpolation of 4 nearest pts w.r.t to x,y from img
    Args:
        img (torch.Tensor): Tensor of size batch_size x width x height. 
        x (torch.Tensor): Float dtype, x axis location for sampling for each batch. Size batch_size x num_points.
        y (torch.Tensor): Float dtype, y axis location for sampling for each batch. Size batch_size x num_points.

    Returns:
        torch.Tensor: interpolated value of size batch_size x num_points
    """
    bs, _, _ = img.shape
