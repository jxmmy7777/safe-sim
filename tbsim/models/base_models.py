...
class HistoryEncoder(nn.Module):
    ...
    def forward(self, lstm_data):
        if len(lstm_data.shape) ==4:
            # If we want to encode multiple agent's history
            b,num_agents,T,dim = lstm_data.shape
            reshaped_lstm_data = lstm_data.view(b*num_agents,T,dim)
            position_lstm_embedding = self._position_lstm(reshaped_lstm_data)[0]
            position_lstm_embedding = position_lstm_embedding.view(b,num_agents,T,-1)[:, :,-1:, :]
            max_pooled_embedding, _ = torch.max(position_lstm_embedding,dim=1)
            return max_pooled_embedding
        else:
            position_lstm_embedding = self._position_lstm(lstm_data)[0][:, -1:, :]
            return position_lstm_embedding
...
class RasterizeROIEncoder(nn.Module):
    ...
    def forward(self, map_inputs: torch.Tensor, rois: torch.Tensor):
        ...
        assert torch.isfinite(pre_roi_feats).all()
        assert torch.isfinite(roi_feats).all()
        assert torch.isfinite(agent_feats).all()
        assert torch.isfinite(global_feats).all()
...
class TrajectoryDecoder(nn.Module):
    ...
    def _create_dynamics(self):
        ...
        elif self._dynamics_type in ["Frenet",dynamics.DynType.FRENET]:
             self.dyn = dynamics.Frenet(
                max_n_dot=self._dynamics_kwargs["max_n_dot"],
                max_t_dot=self._dynamics_kwargs["max_t_dot"]
            )
        else:
            self.dyn = None
...
class GlobalContextAttention(nn.Module):
    def __init__(self, map_feat_dim, feature_dim, attention_dim=128, context_dim=128):
        super(GlobalContextAttention, self).__init__()

        self.map_attention = nn.Sequential(
            nn.Linear(map_feat_dim, attention_dim),
        ...
        self.neigh_attention = nn.Sequential(
            nn.Linear(feature_dim, attention_dim),
            nn.LayerNorm(attention_dim),
            nn.ReLU()
        )

        # Transform the original features into a unified context dimension
        self.map_transform = nn.Linear(map_feat_dim, context_dim)
        self.target_transform = nn.Linear(feature_dim, context_dim)
        self.neigh_transform = nn.Linear(feature_dim, context_dim)

        self.attention_weights = nn.Sequential(
            nn.Linear(attention_dim*3, 3),
            nn.Softmax(dim=-1)  # Apply softmax here for clarity and ensure normalization
        )

    def forward(self, map_feat, target_feat, neigh_feat):
        map_att = self.map_attention(map_feat)
        target_att = self.target_attention(target_feat)
        neigh_att = self.neigh_attention(neigh_feat)

        concat_features = torch.cat([map_att, target_att, neigh_att], dim=-1)
        weights = self.attention_weights(concat_features)

        # Transform features to common context dimension
        map_feat_transformed = self.map_transform(map_feat)
        target_feat_transformed = self.target_transform(target_feat)
        neigh_feat_transformed = self.neigh_transform(neigh_feat)

        global_context = weights[:, 0:1] * map_feat_transformed + weights[:, 1:2] * target_feat_transformed + \
                         weights[:, 2:3] * neigh_feat_transformed
        
        return global_context

    
class RefTrajAttention(nn.Module):
    def __init__(self, global_context_dim, ref_traj_dim, output_dim):
        super(RefTrajAttention, self).__init__()

        # Transforming the global context and reference trajectory dimensions to a common space.
        self.context_transform = nn.Sequential(
            nn.Linear(global_context_dim, output_dim),
            nn.LayerNorm(output_dim),
            nn.ReLU()
        )
        
        self.ref_traj_transform = nn.Sequential(
            nn.Linear(ref_traj_dim, output_dim),
            nn.LayerNorm(output_dim),
            nn.ReLU()
        )

    def forward(self, global_context, ref_traj):
        # Unsqueeze the global context to make it (batch, 1, context_dim)
        transformed_context = self.context_transform(global_context).unsqueeze(1)
        
        # Transform each point in the reference trajectory.
        transformed_ref_traj = self.ref_traj_transform(ref_traj)

        # Calculate the attention scores. 
        # This will result in a tensor of shape (batch, 1, 150) 
        attention_scores = torch.bmm(transformed_context, transformed_ref_traj.transpose(1, 2))
        
        # Compute the attention weights
        weights = F.softmax(attention_scores, dim=2)

        # Apply attention to the transformed reference trajectory.
        attended_ref_traj = torch.bmm(weights, transformed_ref_traj)

        return attended_ref_traj.squeeze(1)
